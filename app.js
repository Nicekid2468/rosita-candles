...
(() => {
  ...
  master.to(".header", { opacity: 1, y: 0, duration: 0.8 });
  ...
  // Badge fades/scales in, then its percentage counts up alongside.
  master.fromTo(
    badgeEl,
  ...
  // Chart container settles into place.
  master.to(
    ".chart-container",
    { opacity: 1, y: 0, scale: 1, duration: 1, ease: "sine.inOut" },
    "-=1.4"
  );

  // Draw the line organically using stroke-dashoffset.
  ...
  // Leading dot fades in and eases toward the final point, mirroring
  // the line draw so it feels like a single continuous gesture.
  ...
  // Asset cards rise in with a gentle stagger, natural momentum feel.
  master.to(
    assetCards,
    {
      opacity: 1,
