...
(() => {
  ...
  master.to(".header", { opacity: 1, y: 0, duration: 0.5 }, 0);

  // Hero label + number rise together with a soft, natural ease.
  master.to(".hero-label", { opacity: 1, y: 0, duration: 0.4 }, 0.2);
  master.to(
    ".hero-value",
  ...
  // below runs IN PARALLEL with the counter so the total video lands
  // right around 8s instead of stacking sequentially.
  master.to(
    ".chart-container",
    { opacity: 1, y: 0, scale: 1, duration: 0.6, ease: "sine.inOut" },
    0.5
  );
  master.fromTo(
    badgeEl,
  ...
  // still the star of the animation, now paced for an 8s video.
  ...
  // Draw the line in sync with the counter climbing.
  ...
  master.to(
    path,
    { strokeDashoffset: 0, duration: 6.5, ease: "power2.inOut" },
    0.7
  );
  ...
  master.to(dot, { opacity: 1, duration: 0.3 }, 0.7);
  master.to(
    dot,
    {
      cx: finalPoint.x,
